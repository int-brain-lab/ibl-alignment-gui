...
data = np.load(
    Path(Path(__file__).parent.parent.joinpath('fixtures', 'alignment_data.npz')),
    allow_pickle=True,
)
...
class TestsEphysAlignment(unittest.TestCase):
    ...
    def test_no_scaling(self):
        xyz_channels = self.ephysalign.get_channel_locations(
            self.feature, self.track, depths=depths
        )
...
class TestsEphysReconstruction(unittest.TestCase):
    def setUp(self):
        self.ephysalign = EphysAlignment(
            xyz_picks, track_prev=track_prev, feature_prev=feature_prev, brain_atlas=brain_atlas
        )
        self.feature = self.ephysalign.feature_init
        self.track = self.ephysalign.track_init
