...
class TestCircularIndexTracker(unittest.TestCase):
    ...
    def test_prev_idx(self):
        ...
        # Once we have moved back make sure the new idx to fill is correct and we reset
        # the total index
        self.tracker.next_idx_to_fill()
        self.assertEqual(self.tracker.current_idx, 1)
        self.assertEqual(self.tracker.idx_prev, 0)
...
class TestAlignmentHandler(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_ephysalign.get_track_and_feature.side_effect = lambda: (
            self.mock_ephysalign.feature_init,
            self.mock_ephysalign.track_init,
            None,
        )
        ...
        ephys_align_patcher = patch(
            'ibl_alignment_gui.handlers.alignment_handler.EphysAlignment',
    ...
    def test_ephys_align_properties(self):
        ...
        np.testing.assert_array_equal(
            self.align_handler.xyz_samples, self.mock_ephysalign.xyz_samples
        )
    ...
    def test_set_init_feature_track(self):
        with self.subTest('No init feature'):
            ...
            np.testing.assert_array_equal(
                self.align_handler.features[idx], self.mock_ephysalign.feature_init
            )
            np.testing.assert_array_equal(
                self.align_handler.tracks[idx], self.mock_ephysalign.track_init
            )
            # Test the feature and track methods work
            np.testing.assert_array_equal(
                self.align_handler.feature, self.align_handler.features[idx]
            )
