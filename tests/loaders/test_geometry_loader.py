...
class TestGeometry(unittest.TestCase):
    ...
    def _mock_geometry(self, mock_data):
        ...
        class MockGeometry(Geometry):
            def _get_n_shanks(self): return mock_data['n_shanks']

            def _get_shank_groups(self): return mock_data['shank_groups']


        return MockGeometry(
            mock_data['x_coords'], mock_data['y_coords'], mock_data['chn_ind']
        )
    ...
    def test_split_sites_per_shank_multi_shank(self):
        ...
        mock_data = {
            ...
            'n_shanks': 3,
            'shank_groups': {0: np.array([0, 1, 2, 3, 4, 5, 13, 14, 15, 16]),
                             1: np.array([6, 7, 8]),
...
class TestGeometryLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_channels = Bunch(localCoordinates=np.array([[0, 0], [10, 10]]),
                                   rawInd=np.array([0, 1]))
    ...
    def _mock_loaders(self, mock_channels, mock_meta):
        ...
        class MockGeometryLoader(GeometryLoader):
            def load_channels(self): return mock_channels

            def load_meta_data(self): return mock_meta
...
class TestArrangeChannelsIntoBanks(unittest.TestCase):
    ...
    def test_single_bank(self):
        ...
        img, scale, offset = arrange_channels_into_banks(shank, np.array([1., 2., 3., 4.]))

        np.testing.assert_array_equal(img, np.array([[1., 2., 3., 4.]]))
    ...
    def test_two_banks(self):
        ...
        np.testing.assert_array_equal(img, np.array([[1., 3.], [2., 4.]]))
        np.testing.assert_array_equal(offset, np.array([0, 0]))
    ...
    def test_checkerboard_layout(self):
        ...
        img, scale, offset = arrange_channels_into_banks(shank, np.arange(1., 9.))
    ...
    def test_gaps_are_nan(self):
        ...
        img, _, _ = arrange_channels_into_banks(shank, np.array([1., 2., 3.]))
        np.testing.assert_array_equal(img, np.array([[1., 2., np.nan, 3.]]))

    def test_bank_width(self):
        """ Test that bnk_width sets the scaling along x """
        shank = make_shank_geom([10] * 4, [0, 20, 40, 60])

        _, scale, _ = arrange_channels_into_banks(shank, np.arange(4.), bnk_width=25)
...
class TestPadDataToFullChnMap(unittest.TestCase):
    ...
    def test_gap_filled_with_nan(self):
        ...
        padded = pad_data_to_full_chn_map(shank, np.array([[1., 2., 3.]]))

        np.testing.assert_array_equal(padded, np.array([[1., 2., np.nan, 3.]]))
